...
//	void execute(vector<string>, Design*) override {
	void execute(vector<string>, RTLIL::Design *design) override {
		log("Hello World!!\n");
//                log("design->refcount_modules_:%u\n",design->refcount_modules_);
//                log("design->modules size:%zu\n",design->modules_.size());
//                log("design->bindings size:%zu\n",design->bindings_.size());
...
////			log("module.second.name:%s\n",my_modules.second->name.c_str());
////			log("Design.Hashidx:%u\n",my_modules.second->design->hashidx_);
////			log("module.second.Hashidx:%u\n",my_modules.second->hashidx_);
////			log("Attributes size:%zu\n",my_modules.second->attributes.size());
////			log("Avail parameters size:%zu\n",my_modules.second->avail_parameters.size());
////			log("Cells size:%zu\n",my_modules.second->cells_.size());
////			log("Connections size:%zu\n",my_modules.second->connections_.size());
////			log("Memories size:%zu\n",my_modules.second->memories.size());
////			log("Monitors size:%zu\n",my_modules.second->monitors.size());
////			log("Param. default values size:%zu\n",my_modules.second->parameter_default_values.size());
////			log("Ports size:%zu\n",my_modules.second->ports.size());
////			log("Wires size:%zu\n",my_modules.second->wires_.size());
////			log("Ref count cells:%i\n",my_modules.second->refcount_cells_);
////			log("Ref count wires:%i\n",my_modules.second->refcount_wires_);
//                    
//                        my_log<<"\n==>MODULE.first:"<<my_modules.first.c_str()<<std::endl;
//			my_log<<"module.second.name:"<<my_modules.second->name.c_str()<<std::endl;
//			my_log<<"Design.Hashidx:"<<my_modules.second->design->hashidx_<<std::endl;
//			my_log<<"module.second.Hashidx:"<<my_modules.second->hashidx_<<std::endl;
//			my_log<<"Attributes size:"<<my_modules.second->attributes.size()<<std::endl;
//			my_log<<"Avail parameters size:"<<my_modules.second->avail_parameters.size()<<std::endl;
//			my_log<<"Cells size:"<<my_modules.second->cells_.size()<<std::endl;
//			my_log<<"Connections size:"<<my_modules.second->connections_.size()<<std::endl;
//			my_log<<"Memories size:"<<my_modules.second->memories.size()<<std::endl;
//			my_log<<"Monitors size:"<<my_modules.second->monitors.size()<<std::endl;
//			my_log<<"Param. default values size:"<<my_modules.second->parameter_default_values.size()<<std::endl;
//			my_log<<"Ports size:"<<my_modules.second->ports.size()<<std::endl;
//			my_log<<"Wires size:"<<my_modules.second->wires_.size()<<std::endl;
//			my_log<<"Ref count cells:"<<my_modules.second->refcount_cells_<<std::endl;
//			my_log<<"Ref count wires:"<<my_modules.second->refcount_wires_<<std::endl;
//		}
////                        my_log<<"\n==>MODULE.first:"<<design->top_module()->c_str()<<std::endl;
//			my_log<<std::endl<<"module.second.name:"<<design->top_module()->name.c_str()<<std::endl;
//			my_log<<"Design.Hashidx:"<<design->top_module()->design->hashidx_<<std::endl;
//			my_log<<"module.second.Hashidx:"<<design->top_module()->hashidx_<<std::endl;
//			my_log<<"Attributes size:"<<design->top_module()->attributes.size()<<std::endl;
//			my_log<<"Avail parameters size:"<<design->top_module()->avail_parameters.size()<<std::endl;
//			my_log<<"Cells size:"<<design->top_module()->cells_.size()<<std::endl;
//			my_log<<"Connections size:"<<design->top_module()->connections_.size()<<std::endl;
//			my_log<<"Memories size:"<<design->top_module()->memories.size()<<std::endl;
//			my_log<<"Monitors size:"<<design->top_module()->monitors.size()<<std::endl;
//			my_log<<"Param. default values size:"<<design->top_module()->parameter_default_values.size()<<std::endl;
//			my_log<<"Ports size:"<<design->top_module()->ports.size()<<std::endl;
//			my_log<<"Wires size:"<<design->top_module()->wires_.size()<<std::endl;
//			my_log<<"Ref count cells:"<<design->top_module()->refcount_cells_<<std::endl;
//			my_log<<"Ref count wires:"<<design->top_module()->refcount_wires_<<std::endl;
//                ModWalker modwalker;
                auto my_modules = design->top_module();
                {
                    log("Hello World!\n");
//                    modwalker.setup( my_modules );
                    SigMap sigmap( my_modules );
                    //dict<RTLIL::SigBit, RTLIL::Cell*> sigbit_to_fanout_index;
                    //dict<RTLIL::SigBit, RTLIL::Cell*> sigbit_to_fanin_index;
//                    dict<RTLIL::SigBit, std::array<RTLIL::Cell*,2> > my_sigbit;
                    log("Hello World!\n");
	            dict<RTLIL::SigBit, std::pair<RTLIL::Cell*, std::vector<RTLIL::Cell*> > > my_sigbit; //considering one source and multiple sinks for each net.
                    
                    log("\nPORTS:\n");
//                    int in_ports_count=0;
//                    for(auto &my_ports : my_modules->ports){
////                        log("port:%s,%u\n", my_ports.c_str(), my_modules->wire(my_ports)->hashidx_);
//                        log("port:%s\n", my_ports.c_str());
//                        if(my_modules->wire(my_ports)->port_input)
//                            in_ports_count++;
//                    }
                    log("Hello World!\n");
                    ...
                    for( auto cell : my_modules->cells() )
                    {
                        ...
                        {
                	    log( "conn fist:%s", conn.first.c_str() );
                            log( ", second as wire:%s\n", conn.second.is_wire() ? conn.second.as_wire()->name.c_str() : "NO" );
                            if( cell->output( conn.first ) ) //is pin an output
                            {
                            	log( "out pin\n" );
                                for( auto bit : sigmap( conn.second ) )
                                {
//                                    if( my_sigbit.find( bit ) == my_sigbit.end() )
//                                        my_sigbit[bit] = std::array<RTLIL::Cell*,2> {cell,NULL};
//                                    else
//                                        my_sigbit[bit][0] = cell;
                                    if( my_sigbit.find( bit ) == my_sigbit.end() )
                                    {
                                        // my_sigbit[bit].first = cell;
                                        // my_sigbit[bit] = std::pair<RTLIL::Cell*, std::vector<RTLIL::Cell*> > ( cell, std::vector<RTLIL::Cell*> ( NULL ) );
                                        my_sigbit[bit] = std::pair<RTLIL::Cell*, std::vector<RTLIL::Cell*> > ( cell, std::vector<RTLIL::Cell*> ( ) );
                                        log( "\tfirst if\n" );
                                    }                                       
                                    else    
                                    {
                                    	//log_error("ERROR, unexpected behaviour!\n");
                                        my_sigbit[bit].first = cell;
                                        log( "\tsecond if\n" );
                                    }
                                }
                            ...
                            if( cell->input( conn.first ) ) //is pin an input
                            {
                                log( "in pin\n" );
                                for( auto bit : sigmap( conn.second ) )
                                {
//                                    if( my_sigbit.find( bit ) == my_sigbit.end() )
//                                        my_sigbit[bit] = std::array<RTLIL::Cell*,2> {NULL,cell};
//                                    else
//                                        my_sigbit[bit][1] = cell;
                                    if( my_sigbit.find( bit ) == my_sigbit.end() )
                                        my_sigbit[bit].second = std::vector<RTLIL::Cell*> {cell};
                                        //my_sigbit[bit] = std::pair<RTLIL::Cell*, std::vector<RTLIL::Cell*> >aux ( NULL, std::vector<RTLIL::Cell*> aux1( cell ) );
                                    else
                                        my_sigbit[bit].second.push_back( cell );
                                }
                            }
                        }
                    }
                    ...
                    for( auto cell : my_modules->cells() )
                    {
//                        for (auto atts: cell->attributes){
//                            my_log<<"--------atts.first.c_str():"<<atts.first.c_str()<<","<<std::endl;
////                            my_log<<"cell:",atts.second.;
//                        }
                        
                        name_to_id.insert( pair<string,int>( cell->name.str(), myID ) );
                        int connCount = 0;

//TODO: fix this to include fanout count of each cell                        
//                        for( auto cellConn : cell->connections_ )
//                        {
//                            log( "cellname(cell):%s\n", cellname(cell).c_str());
//                            log( "cellConn.first:%s\n", cellConn.first);
//                            log( "cellConn.second.size():%u\n", cellConn.second.size());
//                            for( auto wire : cellConn.second )
//                            {
//                                if( wire.is_wire() )
//                                {
//                                    log("wire width:%d\n", wire.wire->attributes );
//                                }
//                            }
//                        }
//                        log("\n\n");
                        
                        connCount = cell->connections_.size();                        
                        cells_file<< myID <<","<< cellname(cell) <<","<< cell->hashidx_ <<","<< cell->type.c_str() <<","<< connCount <<"\n";
//                        dump_cell(cells_file," ",cell);
                        myID++;
                    }
                    
                    int ii=1;
                    log("Wires:\n");
//                    for( auto &my_sigs : my_sigbit )
//                    {
//                        log("\t%u-wire:%s (%u)\n", ii, my_sigs.first.wire->name.c_str(), my_sigs.first.wire->hashidx_ );
//                        log("\tport_id:%u\n", my_sigs.first.wire->port_id );
//                        log("\tport_input:%u\n", my_sigs.first.wire->port_input );
//                        log("\tport_output:%u\n\n", my_sigs.first.wire->port_output );
//                        
//                        //This IF excludes edges from PIs and POs
//                        if( my_sigs.second[0] != NULL && my_sigs.second[1]!=NULL )
//                        {
//                        //WIRE SOURCE
//                            if( my_sigs.second[0] != NULL )
//                            {
//                                //log("\t\t source cell:%s (%u)\n", my_sigs.second[0]->name.c_str(),my_sigs.second[0]->hashidx_ );
//                                log("\t\t edges_file:%u,!\n", name_to_id.at( my_sigs.second[0]->name.c_str() ) );
//                                edges_file<< name_to_id.at( my_sigs.second[0]->name.str() ) <<",";//my_sigs.second[0]->hashidx_<<",";
//                            }
//                            else
//                            {
//                                if(my_sigs.first.wire->port_input==1)
//                                {
//                                    edges_file<< my_sigs.first.wire->port_id <<",";
//                                    log("\t\t edges_file:%s,!\n", my_sigs.first.wire->port_id );
//                            	}
//                            }
//                            //WIRE DESTINATION
//                            if( my_sigs.second[1]!=NULL ){
//                                //log("\t\t destin cell:%s (%u)\n",my_sigs.second[1]->name.c_str(),my_sigs.second[1]->hashidx_);
//                                log("\t\t edges_file:%u,!\n", name_to_id.at(my_sigs.second[1]->name.str()) );
//                                edges_file<< name_to_id.at(my_sigs.second[1]->name.str()) <<",";//<<my_sigs.second[1]->hashidx_<<",";
//                            }
//                            else
//                            {
//    //                            TODO if(my_sigs.first.wire->port_output==1) ISSO NAO EXISTE MEU DEUS 
//                                edges_file<<"-1"<<",";
//                                log("\t\t edges_file:-1,!");
//                            }
//                            edges_file<<"0"<<"\n"; //weight
...
//                    }
                    ...
                    for( auto &my_sigs : my_sigbit )
                    {
                        ...
                        //This IF excludes edges from PIs and POs
                        if( my_sigs.second.first != NULL && my_sigs.second.second.size() > 0 )
                        {
                        //WIRE SOURCE
                            log("\t\t SOURCE!\n");
                            int curr_source;
                            if( my_sigs.second.first != NULL )
                            {
                                //log("\t\t source cell:%s (%u)\n", my_sigs.second[0]->name.c_str(),my_sigs.second[0]->hashidx_ );
                                log("\t\t\t 1edges_file:%u,!\n", name_to_id.at( my_sigs.second.first->name.c_str() ) );
                                //edges_file<< name_to_id.at( my_sigs.second.first->name.str() ) <<",";//my_sigs.second[0]->hashidx_<<",";
                                curr_source = name_to_id.at( my_sigs.second.first->name.str() );
                            }
                            else
                            {
                                if(my_sigs.first.wire->port_input==1)
                                {
                                    log("\t\t\t 2edges_file:%u,!\n", my_sigs.first.wire->port_id );
                                    //edges_file<< my_sigs.first.wire->port_id <<",";
                                    curr_source = my_sigs.first.wire->port_id;
                            	}
                            }
                            edges_file<< curr_source <<",";
                            //WIRE DESTINATION
                            log("\t\t DESTINATION!\n");
                            log("\t\t my_sigs.second.second.size():%lu\n", my_sigs.second.second.size() );
                            if( my_sigs.second.second.size() > 0 )//[1] != NULL )
                            {
                                for( long unsigned a = 0; a < my_sigs.second.second.size(); a++ )
                                {
                                    //log("\t\t destin cell:%s (%u)\n",my_sigs.second[1]->name.c_str(),my_sigs.second[1]->hashidx_);
                                    log("\t\t\t edges_file:%u,!\n", name_to_id.at( my_sigs.second.second[a]->name.str() ) );
                                    edges_file<< name_to_id.at( my_sigs.second.second[a]->name.str() ) <<",0\n";//<<my_sigs.second[1]->hashidx_<<",";
                                    if( a < ( my_sigs.second.second.size() -1 ) )
                                    {
                                        log("\t\t\t edges_file:%u,!\n", curr_source );
                                        edges_file << curr_source << ",";
                                    }
                    ...
                    edges_file.close(); cells_file.close();                    
                }
	}
} HelloWorldPass;
